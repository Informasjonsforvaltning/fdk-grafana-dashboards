    ...
    timeSeriesPanel.new('Search time in seconds')
        ...
        + timeSeriesPanel.queryOptions.withTargets([
            prometheusQuery.new(
              'promehteus',
                |||
                    sum by (fdk_service, kubernetes_namespace) (rate(search_seconds_sum{kubernetes_namespace="$namespace"}[5m])*300)
                |||
            )
            + prometheusQuery.withIntervalFactor(2)
            + prometheusQuery.withLegendFormat(|||
    ...
    timeSeriesPanel.new('Suggestion time in seconds')
          ...
          + timeSeriesPanel.queryOptions.withTargets([
              prometheusQuery.new(
                'promehteus',
                  |||
                      sum by (fdk_service, kubernetes_namespace) (rate(search_suggestion_seconds_sum{kubernetes_namespace="$namespace"}[5m])*300)
                  |||
              )
              + prometheusQuery.withIntervalFactor(2)
              + prometheusQuery.withLegendFormat(|||
         ...
         timeSeriesPanel.new('Index time in seconds')
             ...
             + timeSeriesPanel.queryOptions.withTargets([
                 prometheusQuery.new(
                   'promehteus',
                     |||
                         sum by (fdk_service, kubernetes_namespace) (rate(search_index_seconds_sum{kubernetes_namespace="$namespace"}[5m])*300)
                     |||
                 )
                 + prometheusQuery.withIntervalFactor(2)
                 + prometheusQuery.withLegendFormat(|||
           ...
           timeSeriesPanel.new('Delete time in seconds')
                ...
                + timeSeriesPanel.queryOptions.withTargets([
                    prometheusQuery.new(
                      'promehteus',
                        |||
                            sum by (fdk_service, kubernetes_namespace) (rate(search_delete_seconds_sum{kubernetes_namespace="$namespace"}[5m])*300)
                        |||
                    )
                    + prometheusQuery.withIntervalFactor(2)
                    + prometheusQuery.withLegendFormat(|||
