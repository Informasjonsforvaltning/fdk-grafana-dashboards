...
.addPanel(
  timeseries_panel.new(
    ...
    prometheus.target(
      'sum(rate(kafka_topic_partition_current_offset{topic=~"$topic"}[3m])) by (topic)',
...
.addPanel(
  ...
  .addTarget(
    prometheus.target(
      'sum(rate(kafka_consumergroup_current_offset{topic=~"$topic"}[3m])) by (consumergroup, topic)',
      datasource='prometheus',
      intervalFactor=1,
      interval='1s',
